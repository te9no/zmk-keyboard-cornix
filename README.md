...
| --- | --- | --- | --- | --- | --- |
| 1 | NCS | P1.12 / D7 | CS | MOSI | IRQ |
| 2 | SCLK | P1.13 / D8 | SCLK | MISO | I²C SCL |
| 3 | MOTION | P1.14 / D9 | Motion IRQ | DRDY | I²C SDA |
| 4 | SDIO | P1.15 / D10 | MOSI/MISO共用 | SCLK | 未使用 |
| 5 | 3V3 | — | 3.3 V | 3.3 V | 3.3 V |
| 6 | GND | — | GND | GND | GND |

PMW3610では半二重SPIとしてP1.15をMOSI/MISOで共用します。ADS1220 LPPSではP1.12をMOSI、P1.13をMISO、P1.15をSCLKとして使用します。ADS1220のCSはモジュール内でGNDに固定され、DRDYはJ4.3へ配線されますが、起動時の不安定さを避けるためファームウェアはtimed pollingを使用します。IQS9151は400 kHz I²C、アドレス`0x56`で動作し、SDAにP1.14、SCLにP1.13、Active LowのIRQにP1.12を使用します。Madulaの取り付け方向に合わせ、`CONFIG_INPUT_IQS9151_ROTATE_270=y`（時計回り90度）を設定します。ジェスチャーとフィルターの初期値はGeaconPolarisの実装を基準にしています。
...
TB・LPPS版ではレイヤー1（FUNC）が有効な間、ポインタのXY入力を縦横スクロールへ切り替えます。
上操作で上スクロール、右操作で右スクロールとなり、レイヤー1を離すと通常のカーソル操作に戻ります。
スクロール量の初期倍率はTBが1/20、LPPSが1/10です。取り付け方向の補正とLPPSのデッドゾーンは維持します。
TBのレイヤー1では専用のスクロール処理を使い、通常のStudioカーソル調整は適用しません。
IQSのタッチ入力は従来どおりです。

DYA Studioでエンコーダ割り当てを保存済みの場合、その設定がファームウェアの初期値より優先されます。
書き換え後もPage Up/Downになる場合は、Studioで右手エンコーダの保存済み割り当てをスクロールへ変更してください。
